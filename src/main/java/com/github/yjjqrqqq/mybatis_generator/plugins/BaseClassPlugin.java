...
public class BaseClassPlugin extends PluginAdapter {
    ...
    private void generateSupperClass(TopLevelClass topLevelClass) throws Exception {
        ...
        for (InnerEnum innerEnum : topLevelClass.getInnerEnums()) {
            superClassTemplate.addInnerEnum(innerEnum);
        }
        for (InnerClass innerClass : topLevelClass.getInnerClasses()) {
            superClassTemplate.addInnerClass(innerClass);
        }
        superClassTemplate.setVisibility(JavaVisibility.PUBLIC);
        saveFieldAnnotations(superClassTemplate, fileContent);
        file.delete();
        file.createNewFile();
        ...
        topLevelClass.setSuperClass(topLevelClass.getType().getShortName() + BASE_SUFFIX);
        topLevelClass.getImportedTypes().clear();
        topLevelClass.getInnerEnums().clear();
        topLevelClass.getInnerClasses().clear();

        topLevelClass.addImportedType(superClassFullName);
    }
